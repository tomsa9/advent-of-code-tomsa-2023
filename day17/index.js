...
const populateNode = (r,c) => {
    const node = key(r,c);
    graph[node] = {};
    for(let i=r+1, cost=0; i<=r+3 && i<grid.length; i++) {
        cost += Number(grid[i][c]);
        setPath( key("V", r, c), key("H",i,c), cost );
    }
    for(let i=r-1, cost=0; i>=r-3 && i>=0; i--) {
        cost += Number(grid[i][c]);
        setPath( key("V", r, c), key("H",i,c), cost );
    }
    for(let i=c+1, cost=0; i<=c+3 && i<grid[0].length; i++) {
        cost += Number(grid[r][i]);
        setPath( key("H", r, c), key("V",r,i), cost );
    }
    for(let i=c-1, cost=0; i>=c-3 && i>=0; i--) {
        cost += Number(grid[r][i]);
        setPath( key("H", r, c), key("V",r,i), cost );
    }
};
